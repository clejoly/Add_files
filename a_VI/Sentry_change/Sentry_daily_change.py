...
def get_all_sentry_vis(date):
    ...
    if os.path.exists('/Users/cassandralejoly/Cassandra/Cassandra/Spacewatch/add_files/python_add_files/a_VI/Sentry_change/VI_list_'+date+'.txt'):
        os.unlink('/Users/cassandralejoly/Cassandra/Cassandra/Spacewatch/add_files/python_add_files/a_VI/Sentry_change/VI_list_'+date+'.txt')
    
    # Write out the list and save as 'VI_list_datetoday.txt'
    with open('/Users/cassandralejoly/Cassandra/Cassandra/Spacewatch/add_files/python_add_files/a_VI/Sentry_change/VI_list_'+date+'.txt', 'w') as f:
        for item in des_set:
            f.write("%s\n" % item)
            

def in_out(today_objects_file, latest_objects_file, changes_file_name, directory):
    ...
    existing_content = open(changes_file_name, 'r').readlines()
...
def upload_and_remove_previous(changes_file_name, directory):    

    # Set up SFTP connection with key-based authentication
    private_key = paramiko.RSAKey(filename='/Users/cassandralejoly/.ssh/id_rsa')
    transport = paramiko.Transport(('gagarin.lpl.arizona.edu', 22))
    transport.connect(username='swatch', pkey=private_key)
    sftp = paramiko.SFTPClient.from_transport(transport)

    # Transfer the file to the remote computer
    remote_file_path = f'/home/swatch/Cassandra/sentry_monitoring/Sentry_change_list.txt'
    sftp.put(directory+ "/" + changes_file_name, remote_file_path)
...
def main():
    ...
    directory = "/Users/cassandralejoly/Cassandra/Cassandra/Spacewatch/add_files/python_add_files/a_VI/Sentry_change"
    ...
    get_all_sentry_vis(date_today)

    changes_file_name = "Sentry_change_list.txt"

    

    in_out(today_objects_file, latest_objects_file, changes_file_name, directory)    

    upload_and_remove_previous(latest_objects_file, directory)
